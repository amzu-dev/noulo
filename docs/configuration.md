...
| `NOULO_DEVICE` | `auto` | Where models run: `auto`, `cpu`, `gpu` (best accelerator), or a specific one: `coreml` (Apple GPU / Neural Engine), `cuda` (NVIDIA), `directml` (Windows GPUs), `rocm` (AMD). See [gpu.md](gpu.md) |
